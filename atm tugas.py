...
def main():
    ...
    def show_menu_awal():
        ...
        label_judul = tk.Label(show_menu_awal_frame, text="BANK LUBIS", font=("Arial", 20), bg="#000000", fg="white")
        label_judul.pack()

        label_judul2 = tk.Label(show_menu_awal_frame, text="(BANK LUBIS MOBILE)", font=("Arial", 15), bg="#000000", fg="white")
        label_judul2.pack()

        button_login = tk.Button(show_menu_awal_frame, text="Log In", command=show_opsi_login, font=("Arial", 12), bg="#ff00ff")
        button_login.pack(pady=10)

        button_sign_in = tk.Button(show_menu_awal_frame, text="Sign Up", command=show_sign_in_menu, font=("Arial", 12))
    ...
    def show_opsi_login():
        ...
        button_login_norek = tk.Button(login_options_frame, text="Login dengan Rekening", command=show_login_menu, font=("Arial", 12))
        button_login_norek.pack(pady=10)

        button_login_email = tk.Button(login_options_frame, text="Login dengan Email", command=show_email_login_menu, font=("Arial", 12))
        button_login_email.pack(pady=10)

        button_back = tk.Button(login_options_frame, text="Kembali", command=lambda: show_menu_awal(), font=("Arial", 12))
    ...
    def show_login_menu():
        ...
        button_login = tk.Button(login_frame, text="Login", command=lambda: login_norek(entry_norek.get(), entry_pin.get()), font=("Arial", 12))
        button_login.pack(pady=10)

        button_back = tk.Button(login_frame, text="Kembali", command=lambda: show_menu_awal(), font=("Arial", 12))
        button_back.pack(pady=10)
    ...
    def show_email_login_menu():
        ...
        button_login = tk.Button(email_login_frame, text="Login", command=lambda: login_email(entry_email.get(), entry_password.get()), font=("Arial", 12))
        button_login.pack(pady=10)

        button_back = tk.Button(email_login_frame, text="Kembali", command=lambda: show_menu_awal(), font=("Arial", 12))
        button_back.pack(pady=10)
    ...
    def show_sign_in_menu():
        ...
        button_sign_in = tk.Button(sign_in_frame, text="Sign Up", command=lambda: sign_in(entry_email.get(), entry_password.get(), entry_confirm_password.get(), entry_pin.get(), entry_confirm_pin.get()), font=("Arial", 12))
        button_sign_in.pack(pady=10)

        button_back = tk.Button(sign_in_frame, text="Kembali", command=lambda: show_menu_awal(), font=("Arial", 12))
        button_back.pack(pady=10)

        sign_in_frame.place(relx=0.5, rely=0.5, anchor="center")

    def sign_in(email, password, confirm_password, pin, confirm_pin):
        if not re.match(r"[^@]+@[^@]+\.[^@]+", email):
            messagebox.showerror("Error", "Format email tidak valid!")
            return
        if password != confirm_password:
            messagebox.showerror("Error", "Passwords tidak sesuai!")
            return
        if not re.match(r"^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)[A-Za-z\d]{6,}$", password):
            messagebox.showerror("Error", "Password harus mengandung setidaknya satu huruf kapital, huruf kecil, dan angka!")
            return
        if pin != confirm_pin:
            messagebox.showerror("Error", "PIN tidak sesuai!")
            return
        norek = generate_account_number()
        while norek in users_info:
            norek = generate_account_number()
        users_info[norek] = {'pin': int(pin), 'saldo': 0, 'email': email, 'password': password}
        users[norek] = UserInfo.UserInfo(norek, int(pin), 0, email, password)
        messagebox.showinfo("Success", f"Akun berhasil dibuat! Nomor rekening anda : {norek}")
        show_menu_awal()
    ...
    def show_transaction_menu(norek):
        ...
        button_tarik = tk.Button(transaction_frame, text="Tarik Tunai", command=lambda: show_tarik_tunai_menu(norek), font=("Arial", 12))
        button_tarik.pack()

        button_transfer = tk.Button(transaction_frame, text="Transfer Dana", command=lambda: show_transfer_dana_menu(norek), font=("Arial", 12))
        button_transfer.pack(pady=10)

        button_cek_saldo = tk.Button(transaction_frame, text="Cek Saldo", command=lambda: show_cek_saldo_menu(norek), font=("Arial", 12))
        button_cek_saldo.pack()

        button_mutasi = tk.Button(transaction_frame, text="Mutasi Transaksi", command=lambda: show_mutasi_menu(norek), font=("Arial", 12))
        button_mutasi.pack(pady=10)

        button_cancel = tk.Button(transaction_frame, text="Log Out", command=lambda: show_menu_awal(), font=("Arial", 12))
    ...
    def show_tarik_tunai_menu(norek):
        ...
        button_tarik = tk.Button(tarik_tunai_frame, text="Tarik Tunai", command=tarik_tunai, font=("Arial", 12))
        button_tarik.pack(pady=10)

        button_cancel = tk.Button(tarik_tunai_frame, text="Batal Transaksi", command=lambda: show_transaction_menu(norek), font=("Arial", 12))
        button_cancel.pack(pady=10)
    ...
    def show_transfer_dana_menu(norek):
        ...
        button_transfer = tk.Button(transfer_dana_frame, text="Transfer Dana", command=transfer_dana, font=("Arial", 12))
        button_transfer.pack(pady=10)

        button_cancel = tk.Button(transfer_dana_frame, text="Batal Transaksi", command=lambda: show_transaction_menu(norek), font=("Arial", 12))
        button_cancel.pack(pady=10)
    ...
    def show_cek_saldo_menu(norek):
        ...
        button_cek_saldo = tk.Button(cek_saldo_frame, text="Cek Saldo", command=cek_saldo, font=("Arial", 12))
        button_cek_saldo.pack(pady=10)

        button_cancel = tk.Button(cek_saldo_frame, text="Batal Transaksi", command=lambda: show_transaction_menu(norek), font=("Arial", 12))
        button_cancel.pack(pady=10)
    ...
    def show_mutasi_menu(norek):
        ...
        button_5_transaksi = tk.Button(mutasi_frame, text="5 Transaksi Terakhir", command=lambda: show_mutasi(None), font=("Arial", 12))
        button_5_transaksi.pack(pady=10)

        button_3_hari = tk.Button(mutasi_frame, text="3 Hari Terakhir", command=lambda: show_mutasi(3), font=("Arial", 12))
        button_3_hari.pack(pady=10)

        button_7_hari = tk.Button(mutasi_frame, text="7 Hari Terakhir", command=lambda: show_mutasi(7), font=("Arial", 12))
        button_7_hari.pack(pady=10)

        button_cancel = tk.Button(mutasi_frame, text="Batal Transaksi", command=lambda: show_transaction_menu(norek), font=("Arial", 12))
        button_cancel.pack(pady=10)
    ...
    root = tk.Tk()
    root.title("APLIKASI ATM [FADHLAN RAFIF LUBIS (21120124140152)]")
